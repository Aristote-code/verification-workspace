...
- The user wants this prototype optimized as a desktop product, not expanded as a mobile deliverable. Prioritize stable 1280–1600 px desktop layouts and keep the rail/header bound to the viewport.
...
- Final Decision Review is also a modal over the active evidence workspace, not a standalone page. Keep the review results, reimbursement summary, acknowledgement, reject/return/approve actions, close control, and exact SVG progress component inside the dialog.
- Figma node `6144:4748` is the exact source for the review footer action group: three equal-width 40 px buttons with 15 px gaps; neutral border `#E2E4E9`; destructive fill `#DF1C41`, ring `#E93535`, shadow `#AF1D1D7A`; success fill/ring `#1DAF61`, shadow `#1DAF617A`.
- Do not layer extra highlight pseudo-elements over the destructive or success variants. Their Figma fill already includes the complete 12% white vertical highlight.
